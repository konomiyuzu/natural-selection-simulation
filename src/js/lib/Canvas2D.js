...
export class Canvas2DRectangleEntry {
    zIndex;
    args;
    /**
...
export class Canvas2DCircleEntry {
    zIndex;
    args;
    /**
     * @param {Vector2D} position should be in canvas coordinates
...
export class Canvas2DLineEntry {
    zIndex;
    args;
    /**
     *
...
export class Canvas2DPolygonEntry {
    zIndex;
    args;
    /**
     *
...
export default class Canvas2D {
    ...
    queuePoint(position, zIndex = 0, color = "#FFFFFF") {
        const entry = new Canvas2DRectangleEntry(this.vectorCoordinatesToCanvasCoordinates(position), new Vector2D(1, 1), zIndex, color);
    ...
    queueCenteredRectangle(position, dimensions, zIndex = 0, color = "#FFFFFF") {
        const centeredPosition = new Vector2D(position.x - (dimensions.x / 2), position.y + (dimensions.y / 2));
    ...
    queueSquare(position, size, zIndex = 0, color = "#FFFFFF") {
        const entry = new Canvas2DRectangleEntry(this.vectorCoordinatesToCanvasCoordinates(position), new Vector2D(size, size), zIndex, color);
        this.addToDrawQueue(entry);
    }
    /**
     *
    ...
    queueCenteredSquare(position, size, zIndex = 0, color = "#FFFFFF") {
        const centeredPosition = new Vector2D(position.x - (size / 2), position.y + (size / 2));
        const entry = new Canvas2DRectangleEntry(this.vectorCoordinatesToCanvasCoordinates(centeredPosition), new Vector2D(size, size), zIndex, color);
        this.addToDrawQueue(entry);
    }
    /**
     *
